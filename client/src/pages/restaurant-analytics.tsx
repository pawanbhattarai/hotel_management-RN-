...
import { 
  TrendingUp, DollarSign, Utensils, Clock, 
  ChefHat, Users, Star, TrendingDown 
} from "lucide-react";
...
export default function RestaurantAnalytics() {
  ...
  if (revenueLoading || ordersLoading || dishesLoading || tablesLoading || operationsLoading) {
    return (
              ...
              <div>
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className={`text-sm font-medium ml-1 ${(revenueData as any)?.revenueGrowth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {(revenueData as any)?.revenueGrowth?.toFixed(1)}% vs last period
                  </span>
                </div>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">
                  NPR {(ordersData as any)?.averageOrderValue?.toLocaleString() || '0'}
                </p>
                <div className="flex items-center mt-2">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    ...
                    >
                      {ordersData?.ordersByStatus?.map((entry: any, index: number) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {dishesData?.topDishes?.slice(0, 5).map((dish: any, index: number) => (
                    <div key={dish.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{dish.totalOrders} orders</p>
                        </div>
                      </div>
                      <span className="text-lg font-bold text-green-600">NPR {dish.totalRevenue}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={dishesData?.categoryPerformance || []}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="totalRevenue" fill="#f59e0b" name="Revenue (NPR)" />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
        ...
        <TabsContent value="tables" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={tablesData?.tablePerformance || []}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="totalRevenue" fill="#06b6d4" name="Revenue (NPR)" />
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {tablesData?.tableUtilization?.slice(0, 5).map((table: any, index: number) => (
                    <div key={table.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="text-lg font-bold text-blue-600">{table.utilizationRate}%</span>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-lg font-bold">{operationsData?.avgPreparationTime} min</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="font-medium">Average Service Time</span>
                    <span className="text-lg font-bold">{operationsData?.avgServiceTime} min</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="font-medium">Order Cancellation Rate</span>
                    <span className="text-lg font-bold text-red-600">{operationsData?.cancellationRate}%</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="font-medium">Customer Satisfaction</span>
                    <span className="text-lg font-bold text-green-600">{operationsData?.customerSatisfaction}/5</span>
                  </div>
                </div>
              </CardContent>
            </Card>
