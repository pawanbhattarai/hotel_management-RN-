...
export default function Analytics() {
  ...
  if (revenueLoading || occupancyLoading || guestLoading || roomLoading || operationsLoading) {
    return (
                ...
                <div className="space-y-4">
                  {occupancyData?.peakOccupancyDays?.slice(0, 5).map((day: any, index: number) => (
                    <div key={day.date} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="text-lg font-bold text-green-600">{day.occupancyRate}%</span>
                    </div>
                  ))}
                </div>
              </CardContent>
        ...
        <TabsContent value="guests" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    ...
                    >
                      {guestData?.guestsByNationality?.map((entry: any, index: number) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {guestData?.topGuests?.slice(0, 5).map((guest: any, index: number) => (
                    <div key={guest.guest.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 bg-purple-500 rounded-full flex items-center justify-center text-white font-bold text-sm">
                          {index + 1}
                        </div>
                        <div>
                          <span className="font-medium">{guest.guest.firstName} {guest.guest.lastName}</span>
                          <p className="text-sm text-gray-500">{guest.totalBookings} bookings</p>
                        </div>
                      </div>
                      <span className="text-lg font-bold text-green-600">NPR {guest.totalSpent}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={roomData?.roomTypePerformance || []}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="totalRevenue" fill="#8884d8" name="Revenue (NPR)" />
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-lg font-bold">{operationsData?.averageStayDuration} days</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-lg font-bold text-orange-600">{operationsData?.noShowRate}%</span>
                  </div>
                </div>
              </CardContent>
            </Card>
