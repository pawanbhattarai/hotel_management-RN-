...
export default function RecentOrders() {
  const { data: orders, isLoading, error } = useQuery({
    ...
    queryFn: () => fetch("/api/restaurant/dashboard/today-orders?limit=5").then(res => res.json()),
  });
    ...
    <Card className="bg-white rounded-xl shadow-sm border border-gray-200">
      <CardHeader className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold text-gray-900">Today's Orders</CardTitle>
          <a href="/restaurant-orders" className="text-primary text-sm font-medium hover:text-primary/80">
            View All
          </a>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <div className="px-6 py-8 text-center">
            <p className="text-gray-500">No orders made today. Create your first order to get started.</p>
          </div>
        ) : (
