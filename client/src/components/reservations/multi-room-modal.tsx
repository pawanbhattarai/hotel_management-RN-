...
export default function MultiRoomModal({ isOpen, onClose }: MultiRoomModalProps) {
  const { user } = useAuth();
  ...
  const updateRoom = (index: number, field: keyof RoomData, value: any) => {
    ...
    if (field === "roomTypeId" || field === "checkInDate" || field === "checkOutDate") {
      const roomType = roomTypes?.find((rt: any) => rt.id === parseInt(updatedRooms[index].roomTypeId));
      if (roomType && updatedRooms[index].checkInDate && updatedRooms[index].checkOutDate) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidRoom = rooms.find(room => !room.roomTypeId || !room.checkInDate || !room.checkOutDate);
    if (invalidRoom) {
      toast({
        title: "Validation Error",
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <SelectContent>
                          {roomTypes?.map((roomType: any) => (
                            <SelectItem key={roomType.id} value={roomType.id.toString()}>
                              {roomType.name} - ${parseFloat(roomType.basePrice).toFixed(2)}/night
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateRoom(index, "checkInDate", e.target.value)}
                        required
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateRoom(index, "checkOutDate", e.target.value)}
                        required
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => updateRoom(index, "children", parseInt(value))}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
          ...
          <Card className="bg-gray-50">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between border-t border-gray-300 pt-2">
                ...
                <span className="font-bold text-lg text-primary">${summary.total.toFixed(2)}</span>
              </div>
            </CardContent>
          </Card>
